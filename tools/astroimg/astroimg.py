...
def z80Exporter(newimg):
    ...
    x = 0
    for byte in range(len(export)):
        while x <= 4070:
            z80 = z80 + "\nDB " + str(export[x]) + ", " + str(export[x+1]) + ", " + str(export[x+2]) + ", " + str(export[x+3]) + ", " + str(export[x+4]) + ", " + str(export[x+5]) + ", " + str(export[x+6]) + ", " + str(export[x+7]) + ", " + str(export[x+8]) + ", " + str(export[x+9])
            x = x+10
    newFile = open(graphicsFilename, "wb")
    newFile.write(z80)
    newFile.close()
...
PrgStart:   DI                      ; Disable interrupts
            ...
            DB      102*2           ; ... with VBLANK line set to line 102
"""
