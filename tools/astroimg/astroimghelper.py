...
#rgb2hsv: Helper to convert RGB to HSV
#eightColorDownsample: Downsamples image to a maximum of 8 colors
#colorBoundaryFinder: Calculates position of color boundary
#colorCounter
#colorRoller: If there are more than 4 colors on one side of color boundary, reduce down to four
#colorSwapper: Helper for colorRoller to rewrite pixel values
    ...
    for y in range(img.size[1]):
        for x in range(img.size[0]):
            ...
            if pixelMap[x,y] == color_to_roll:
                img.pixelMap[x,y] = color_to_keep
